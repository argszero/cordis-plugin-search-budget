...
// Type-only: each of these loads a package's Cordis declaration merging —
// `tools/pre-execute` (dsh-tools), `session/disposed` (dsh-session), and the
// `agents` service (dsh-agent) — without importing any runtime value. The
// plugin therefore has no runtime dependency on the harness packages.
...
/* -------------------------------------------------------------------------- */
/* Seam shapes (structural — no runtime dependency on dsh packages)            */
/* -------------------------------------------------------------------------- */
 ...
 */
export function countQueries(raw: unknown): number {
 ...
 */
export function resolveScope(agent: AgentLike, lookup: AgentLookup): AgentScope {
...
export function resolveRootId(
  ...
  for (;;) {
    ...
    if (parentId === undefined || seen.has(parentId)) return currentId
    seen.add(parentId)
    currentId = parentId
 ...
 */
export function currentTurn(session: SessionLike): number {
  if (session.snapshotEvents === undefined) return 0
  const events = session.snapshotEvents(0, session.seq)
  for (let index = events.length - 1; index >= 0; index -= 1) {
    const event = events[index]
    if (typeof event !== 'object' || event === null) continue
    const { type, data } = event as { type?: unknown; data?: { turn?: unknown } }
...
export class SearchBudget {
  private readonly sessions = new Map<string, Bucket>()
  private readonly trees = new Map<string, Bucket>()
  private readonly turns = new Map<string, Bucket>()
  ...
  admit(scope: AgentScope, queries: number): Admission {
    ...
    const checks: readonly { readonly scope: string; readonly limit: number; readonly bucket: Bucket | undefined }[] = [
  ...
  charge(scope: AgentScope, queries: number): number {
    const cost = this.costOf(queries)
    if (this.config.perTurn > 0) bucketFor(this.turns, turnKey(scope)).spent += cost
    if (this.config.perSession > 0) bucketFor(this.sessions, scope.sessionId).spent += cost
    if (this.config.perTree > 0) bucketFor(this.trees, scope.rootId).spent += cost
    return cost
  }
...
function turnKey(scope: AgentScope): string {
 ...
 */
export function denialReason(refusal: Refusal, cost: number): string {
...
export function apply(ctx: Context, config: Config): void {
  ...
  const lookup: AgentLookup = (id) => ctx.get('agents')?.get(id as never)

  ctx.on('tools/pre-execute', async (exec, next) => {
    if (exec.name !== resolved.toolName) return next()
    ...
    if (!admission.admitted) {
      ...
      ctx.logger?.warn(
        `${DENIAL_PREFIX} (${admission.refusal?.scope ?? 'unknown'}, observe-only): `
